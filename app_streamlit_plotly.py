...
Versão otimizada com Plotly (sem matplotlib)
"""
...
def main():
    ...
    # Gráfico Plotly - Distribuição de scores
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=('Contagem de Reviews por Score', 'Distribuição Percentual por Score'),
        specs=[[{"type": "bar"}, {"type": "bar"}]]
    )
    
    # Contagem absoluta
    fig.add_trace(
        go.Bar(x=score_counts.index, y=score_counts.values, name='Contagem', 
    ...
    # Porcentagem
    fig.add_trace(
        go.Bar(x=score_percentages.index, y=score_percentages.values, name='Porcentagem', 
    ...
    fig.update_layout(
        height=400,
        showlegend=False,
        title_text="Distribuição de Scores dos Reviews"
    )
    
    fig.update_xaxes(title_text="Score", row=1, col=1)
    fig.update_xaxes(title_text="Score", row=1, col=2)
    fig.update_yaxes(title_text="Quantidade de Reviews", row=1, col=1)
    ...
    **💡 Interpretação:** 
    ...
    if len(df_year) > 0:
        ...
        # Gráfico Plotly - Tendência temporal
        fig = go.Figure()
        
        fig.add_trace(go.Scatter(
            x=monthly_avg['Mês'],
            y=monthly_avg['Score Médio'],
            mode='lines+markers',
            name='Score Médio',
    ...
    with col1:
        ...
        if len(positive_reviews) > 0:
            ...
            # Gráfico Plotly - Palavras positivas
    ...
    with col2:
        ...
        if len(negative_reviews) > 0:
            ...
            # Gráfico Plotly - Palavras negativas
    ...
    <div style='text-align: center; color: #666;'>
        ...
        <p>💡 Potencializado por banco vetorial FAISS para busca semântica</p>
        <p>🚀 Versão otimizada com Plotly (sem matplotlib)</p>
    </div>
    """, unsafe_allow_html=True)
